def main():
    ...
    parser.add_argument('--target_prob_csv', type=str, default="", help='Path to save test target/prob CSV')
...
def train(args):
    ...
    a = Agent(args)
    run = wandb.init(entity='faultdiagnosis', project='faultdiagnosis-issre2026', config=wandb_config)

    a.train()
    run.finish()

def export_target_prob_csv(test_df, args):
    import csv
    import os
    import numpy as np

    targets = np.reshape(np.concatenate(test_df['target'].to_numpy()), (-1, len(args.classes))).astype(int)
    probs = np.concatenate(test_df['prob'].to_numpy())

    csv_path = args.target_prob_csv or f"{args.model}_target_prob.csv"
    csv_dir = os.path.dirname(csv_path)
    if csv_dir:
        os.makedirs(csv_dir, exist_ok=True)

    with open(csv_path, 'w', newline='') as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=['true_label', 'predicted_prob', 'class', 'model'])
        writer.writeheader()
        ...
        for class_id in range(n_classes):
            ...
            for sample_id in range(n_samples):
                writer.writerow({
                    'true_label': int(targets[sample_id, class_id]),
                    'predicted_prob': float(probs[sample_id, class_id]),
                    'class': class_name,
                    'model': args.model,
                })

    print(f'Saved target/prob CSV to {csv_path}')
...
def evaluate(args):
    ...
    export_target_prob_csv(test_df, args)
